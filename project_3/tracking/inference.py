...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.allPositions:
            emission = self.getObservationProb(noisyDistance, pacman_pos, p, self.getJailPosition())
            if emission >= 0:
                self.beliefs[p] *= emission
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            transition[oldPos] = self.getPositionDistribution(gameState, oldPos)
        for p in self.allPositions:
            new_beliefs[p] = sum(transition[prevPosition][p] * beliefs[prevPosition] for prevPosition in beliefs)
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if tmp.total() == 0:
            ...
            self.particles = [tmp.sample() for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            particles.append(self.getPositionDistribution(gameState, p).sample())
