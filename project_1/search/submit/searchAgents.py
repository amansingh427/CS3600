    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                ...
                successors.append(((successor, tmp), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in visited:
            dvisited.append(corner)

    while dvisited:
        distanceList = []
        for corner in dvisited:
            distanceList.append((util.manhattanDistance(curr, corner), corner))
        distance, corner = min(distanceList)
        heuristic += distance
        curr = corner
        dvisited.remove(corner)
    return heuristic
...
def foodHeuristic(state, problem):
    ...
    for food in food_list:
        heuristic = max(heuristic, util.manhattanDistance(position, food))
    return heuristic
